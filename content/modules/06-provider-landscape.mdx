...
**Image models** — these generate, edit, or describe images. Midjourney, ChatGPT Images, Flux, Adobe Firefly, Nano Banana. We will look at these more closely in the next module.
...
## The tools worth knowing now

Without endorsing specific versions (which will be outdated), there are four areas of the landscape worth being aware of:
...
**Gemini (Google)** — particularly well-suited if you work within Google's ecosystem (Docs, Drive, Gmail). Gemini 3 models have strong multimodal capabilities and power Google's Nano Banana image tool.

**Image generators** — Midjourney, ChatGPT Images, Flux, Adobe Firefly, and Nano Banana are all worth knowing. Each has a different character. Not for final deliverables, but excellent for generative inspiration and client communication. We cover these in depth in Module 8.

The instinct to find the "best" single tool is understandable but usually not useful. Most people who work with AI professionally use two or three tools, each for different tasks. The skill is knowing which task goes to which tool — and that comes from trying them, not reading about them.
